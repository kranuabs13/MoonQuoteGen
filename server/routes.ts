...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    const allowedExtensions = ['.pdf', '.xls', '.xlsx', '.eml', '.msg'];
    ...
    if (hasValidType || hasValidExtension) {
      ...
      cb(new Error('Invalid file type. Only PDF, Excel, and email files are allowed.'));
    }
  }
});

// PDF parsing function - simplified approach
async function parsePDFContent(buffer: Buffer): Promise<string> {
  try {
    // For now, return a message that PDF parsing requires manual extraction
    // In a production system, you would use a robust PDF parsing service
    const bufferString = buffer.toString('binary');
    
    // Try to extract basic text content from PDF stream
    ...
    // If no meaningful text extracted, return a placeholder
    if (extractedText.trim().length < 50) {
      extractedText = `PDF content detected - ${buffer.length} bytes. Please extract text manually and paste into a text file or email for better parsing results.`;
    }
    
    return extractedText.trim();
  } catch (error) {
    console.error('PDF parsing error:', error);
    return `PDF parsing error - please convert to text or Excel format for better results. File size: ${buffer.length} bytes`;
  }
}
...
function parseEmailContent(buffer: Buffer, filename: string): string {
  try {
    let content = buffer.toString('utf-8');
    
    if (filename.toLowerCase().endsWith('.eml')) {
      // Basic EML parsing - extract content after headers
      ...
      // Basic MSG file handling - treat as text for now
      return content;
    }
...
// Intelligent data extraction function
function extractProductData(text: string, filename: string): any {
  const lines = text.split('\n').filter(line => line.trim().length > 0);
  
  // Try to identify supplier from filename or content
  let supplier = 'Unknown Supplier';
  
  // Extract supplier from filename patterns
  ...
  // Extract items with prices using various patterns
  const items: any[] = [];
  const pricePatterns = [
    // Pattern: Description | Qty | Unit Price | Total
    /(.+?)\s+(\d+(?:\.\d+)?)\s+(?:₪|NIS|USD|\$)?\s*(\d+(?:,\d{3})*(?:\.\d{2})?)\s+(?:₪|NIS|USD|\$)?\s*(\d+(?:,\d{3})*(?:\.\d{2})?)/g,
    // Pattern: Description - Price
    /(.+?)\s*-\s*(?:₪|NIS|USD|\$)?\s*(\d+(?:,\d{3})*(?:\.\d{2})?)/g,
    // Pattern: Qty x Description @ Price = Total
    /(\d+(?:\.\d+)?)\s*x?\s*(.+?)\s*@\s*(?:₪|NIS|USD|\$)?\s*(\d+(?:,\d{3})*(?:\.\d{2})?)\s*=?\s*(?:₪|NIS|USD|\$)?\s*(\d+(?:,\d{3})*(?:\.\d{2})?)?/g
  ];
  ...
  for (const pattern of pricePatterns) {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      if (description && description.length > 3) {
        ...
        const qty = match[2] ? parseFloat(match[2].replace(/,/g, '')) : 1;
        const unitPrice = match[3] ? parseFloat(match[3].replace(/,/g, '')) : 0;
        const totalPrice = match[4] ? parseFloat(match[4].replace(/,/g, '')) : unitPrice * qty;
        ...
        if (unitPrice > 0 || totalPrice > 0) {
          items.push({
            ...
            quantity: qty || 1,
            unitPrice: unitPrice || (totalPrice / (qty || 1)),
            totalPrice: totalPrice || (unitPrice * (qty || 1)),
            unit: 'units'
          });
        }
      }
  ...
  // If no structured data found, try to extract any price information
  if (items.length === 0) {
    const fallbackPatterns = [
      /(?:₪|NIS|USD|\$)\s*(\d+(?:,\d{3})*(?:\.\d{2})?)/g,
      /(\d+(?:,\d{3})*(?:\.\d{2})?)\s*(?:₪|NIS|USD|\$)/g
    ];
    ...
    // Create generic items for found prices
    foundPrices.forEach((price, index) => {
      items.push({
        id: `item-${index + 1}`,
        description: `Item ${index + 1} (from ${filename})`,
        quantity: 1,
        unitPrice: price,
        totalPrice: price,
        unit: 'units'
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Import routes - File parsing for supplier quotes
  app.post("/api/import/parse", upload.array('files'), async (req, res) => {
    try {
      console.log('=== POST /api/import/parse HANDLER CALLED ===');
      ...
      let allItems: any[] = [];
      let combinedSupplier = 'Multiple Suppliers';
      let totalValue = 0;
      ...
      for (const file of files) {
        ...
        try {
          ...
          if (file.mimetype === 'application/pdf') {
            content = await parsePDFContent(file.buffer);
          } else if (file.mimetype.includes('spreadsheet') || file.mimetype.includes('excel') || filename.endsWith('.xlsx') || filename.endsWith('.xls')) {
            content = parseExcelContent(file.buffer);
          } else if (file.mimetype === 'message/rfc822' || filename.endsWith('.eml') || filename.endsWith('.msg')) {
            content = parseEmailContent(file.buffer, filename);
          } else {
          ...
          const extractedData = extractProductData(content, filename);
          
          if (extractedData.items.length > 0) {
            // Prefix items with file name for identification
            const prefixedItems = extractedData.items.map((item: any) => ({
              ...item,
              id: `${filename.replace(/\.[^/.]+$/, '')}-${item.id}`,
            ...
            if (files.length === 1) {
              combinedSupplier = extractedData.supplier;
            }
            
            console.log(`Extracted ${extractedData.items.length} items from ${filename}`);
          }
      ...
      totalValue = allItems.reduce((sum: number, item: any) => sum + item.totalPrice, 0);
      
      const result = {
        supplier: combinedSupplier,
        items: allItems,
        totalValue: totalValue,
        fileCount: files.length
      };
      
      console.log(`Successfully processed ${files.length} files, extracted ${allItems.length} total items`);
      
      res.json(result);
      
    } catch (error) {
      console.error('Error in import/parse route:', error);
      res.status(500).json({ 
